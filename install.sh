...
sudo apt-get install -y python-dev python-numpy python-opencv

cd ~/src/
git clone https://github.com/fwestling/comma.git
cd ~/build/comma
# ccmake ~/src/comma
cmake ~/src/comma && make -j 4 && sudo make install
...
cd ~/src/
git clone https://github.com/fwestling/qt3d.git
cd qt3d
...
cd ~/src/
git clone https://github.com/fwestling/snark.git
cd ~/build/snark
# ccmake ~/src/snark
sudo cmake -D snark_build_imaging=OFF ~/src/snark && make -j 4 && sudo make install


# And then my dependencies!!
sudo apt -y install parallel
sudo apt -y install python-pip
pip install -Iv Pysolar===0.6
